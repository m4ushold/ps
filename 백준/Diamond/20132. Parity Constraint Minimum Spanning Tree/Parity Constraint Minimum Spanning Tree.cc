...
ll N, M, D[101010], P[18][101010], MX[18][101010], MX2[18][101010];
vector<pair<ll,ll>> G[101010];
void dfs(ll v=1, ll b=-1) { 
    for(auto [i,w]:G[v]) if(i!=b) {
        if(w&1) MX[0][i]=w, MX2[0][i]=-1;
        else MX[0][i]=-1, MX2[0][i]=w;
...
int main() {
    ...
    if(S[Find(1)]!=N) return cout << "-1 -1", 0;
    ...
    for(auto [w,s,e]:ex) {
        ...
        if(w1>=0) {
            ll tmp = mst - w1 + w;
            if(tmp & 1) odd = min(odd, tmp);
            else even = min(even, tmp);
        }
        if(w2>=0) {
            ll tmp = mst - w2 + w;
            if(tmp & 1) odd = min(odd, tmp);
