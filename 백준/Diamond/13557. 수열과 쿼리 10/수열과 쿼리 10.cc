...
void sol(int tc) {
    ...
    for(int i=1,x;i<=n;i++) cin >> x, seg.set(i,S(x));
    int q; cin >> q;
    for(int i=0,a,b,c,d;i<q;i++) {
        cin >> a >> b >> c >> d;
        if(b<c) {
            cout << seg.qry(b,c).sum + max(0LL,seg.qry(a,b-1).r) + max(0LL,seg.qry(c+1,d).l) << endl;
        } else {
            ll aa = seg.qry(c,b).mx;
            ll bb = seg.qry(a,c-1).r + seg.qry(c,d).l;
            ll cc = seg.qry(b+1,d).l + seg.qry(a,b).r;
